...
public class PageRegister extends BasePageWithTitle {
	...
	protected View onCreateContentView(Context context) {
		View view = LayoutInflater.from(context).inflate(getLayoutId("bbs_page_account_register"), null);
		return view;
	}
	...
	private void register() {
		...
		BBSSDK.getApi(UserAPI.class).register(strname, pwd, strmail, false, new APICallback<User>() {
			...
			public void onSuccess(API api, int action, User result) {
				...
				setResult(map);
				PageInitProfile page = BBSViewBuilder.getInstance().buildPageInitProfile();
				page.initPage(result);
			...
			public void onError(API api, int action, int errorCode, Throwable details) {
				...
				if (errorCode == ErrorCode.SDK_API_ERROR_USER_NOT_ACTIVATED) {
					PageRegisterConfirm page = BBSViewBuilder.getInstance().buildPageRegisterConfirm();
					page.initPage(strname, strmail);
					page.show(activity);
				} else {
