...
public class FollowingsPullRequestView extends BBSPullToRequestView<Following> {
	...
	public FollowingsPullRequestView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public FollowingsPullRequestView(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void init() {
		...
		setOnRequestListener(new BBSPullToRequestView.OnRequestListener() {
			...
			public void onRequest(int page, final BasePagedItemAdapter.RequestCallback callback) {
				...
				userAPI.getFollowingList(nUserID, page, nDefaultLoadOnceCount, false, new APICallback<List<Following>>() {
					@Override
					public void onSuccess(API api, int action, List<Following> result) {
	...
	protected View getContentView(int position, View convertView, ViewGroup parent) {
		if (convertView == null) {
			convertView = ListViewItemBuilder.getInstance().buildFollowingsItemView(convertView, parent);
			if(convertView == null) {
				convertView = layoutInflater.inflate(ResHelper.getLayoutRes(getContext(), "bbs_item_following"), parent, false);
			}
		}
